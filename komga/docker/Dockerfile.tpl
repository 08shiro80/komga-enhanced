...
RUN apt-get -y update && \
    apt-get -y install --no-install-recommends ca-certificates locales libjxl-dev libheif-dev libwebp-dev libarchive-dev wget curl python3 python3-pip && \
    echo "en_US.UTF-8 UTF-8" >> /etc/locale.gen && \
    locale-gen en_US.UTF-8 && \
    pip3 install --break-system-packages --no-cache-dir gallery-dl && \
...
RUN apt-get -y update && \
    apt-get -y install --no-install-recommends wget curl python3 python3-pip && \
    pip3 install --break-system-packages --no-cache-dir gallery-dl && \
    apt-get -y autoremove && rm -rf /var/lib/apt/lists/*
COPY --from=kepubify-builder /go/bin/kepubify /usr/bin/kepubify
