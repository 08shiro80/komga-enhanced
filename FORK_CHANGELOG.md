...
|-------------------|---------|
| `domain/service/BookLifecycle.kt` | `analyzeAndPersist` now calls `restoreHashFrom(previous.pages)` before persisting — preserves existing page hashes across re-analyze, stops Duplicate Pages count from collapsing during scans |
