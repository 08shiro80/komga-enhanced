...
### Bug Fixes
- **Series "last modified" sorting used series folder date instead of newest book date** — Sorting by `lastModifiedDate` used the Series entity's `LAST_MODIFIED_DATE` column (when the series folder/metadata was last touched), so a series whose `series.json` was rewritten appeared before one that got a new chapter. Now uses a scalar subquery `MAX(book.LAST_MODIFIED_DATE)` to sort by the most recently added/updated book in the series.
