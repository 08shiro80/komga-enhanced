...
| `komga-webui/src/views/OversizedPages.vue` | Rewritten from `v-data-table` to card grid (`v-row` + `v-slide-x-transition` + 220×320 `v-img`), custom pagination/sort controls; toolbar buttons hide text labels on xs, dialogs fullscreen on xs; `splitPreviewParts` returns `2` in WIDE mode to match `ImageSplitter.splitWideImage` halving behavior; split-ratio hint and confirm-dialog copy reworded accordingly; new debounced search field wired to `search=` query param |
